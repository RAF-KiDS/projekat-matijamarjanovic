...
public class SnapshotCollectorWorker implements SnapshotCollector {
	...
	public void run() {
		while(working) {
			...
			//set current initiator
			AppConfig.snapshotInfo.setInitiatorId(AppConfig.myServentInfo.getId());
			((LaiYangBitcakeManager)bitcakeManager).markerEvent(AppConfig.myServentInfo.getId(), this, AppConfig.snapshotInfo.getSnapshotNo().get());
			...
			//print SNAPSHOT
			Snapshot snapshot = new Snapshot(AppConfig.myServentInfo.getId(), AppConfig.snapshotInfo.getSnapshotNo().getAndIncrement());
			int sum;
			sum = 0;
			for (Entry<Integer, LYSnapshotResult> nodeResult : collectedLYValues.entrySet()) {
				...
				//dodavnje u snapshot
				snapshot.getCollectedLYValues().put(nodeResult.getKey(), nodeResult.getValue());
			}
					...
					if (i != j) {
						if (AppConfig.getInfoById(i).getNeighbors().contains(j) &&
							...
							if (ijAmount != jiAmount) {
								...
								//dodavanje u snapshot TRANZIT polje u knjizi
								snapshot.setMessagesInTransit(snapshot.getMessagesInTransit() + ijAmount - jiAmount);
			...
			//Reset system
			((LaiYangBitcakeManager) bitcakeManager).whiteMarkerEvent(AppConfig.myServentInfo.getId());
	...
	@Override
	public void stop() {
		AppConfig.collectingInfo.set(collecting.get());
		working = false;
	}
